...
export default function AddDevice() {
    ...
    const handleCreate = (e: FormEvent) => {
        ...
        if (nameRef.current && powerRef.current && codeRef.current) {
            ...
            if (formData) {
                // Kết nối tới server thông qua socket.io-client
                const socket = io('http://localhost:3005'); // Thay 'http://your-server-address' bằng địa chỉ thực tế của server

                // Gửi dữ liệu formData qua socket khi nút được nhấn
                socket.emit('addDevices', formData);

                // Đóng kết nối sau khi gửi xong (nếu cần)
                // socket.disconnect();
            }
        }
    }
